...
public class HelloController {
    ...
    public void handleStart() {
        ...
        // board = new
        // GridSpot[selectedStage().getSideLength()][selectedStage().getSideLength()];
        // boardSpotsBTN = new
        // Button[selectedStage().getSideLength()][selectedStage().getSideLength()];
        resetEverything();
        handleLoad();
    ...
    public void putWord(int row, int col, String word, int direction) {
        ...
        if (!(top && right && bottom && left && topRight && bottomRight && bottomLeft && topLeft)
                && !board[row][col].isChecked()) {
            // UP
            if (direction == 0) {
    ...
    public boolean canPut(String word, int rowNum, int colNum, int direction) {
        ...
        if (direction == 0) {
            for (int i = 0; i < word.length(); i++) {
                if (board[rowNum - i][colNum].getBtnText().equals("-")
                        || board[rowNum - i][colNum].getBtnText().equals(word.substring(i, i + 1))) {
                    canNum++;
                }
            }
        }
        if (direction == 1) {
            for (int i = 0; i < word.length(); i++) {
                if (board[rowNum][colNum + i].getBtnText().equals("-")
                        || board[rowNum][colNum + i].getBtnText().equals(word.substring(i, i + 1))) {
                    canNum++;
                }
            }
        }
        ...
        if (direction == 4) {
            for (int i = 0; i < word.length(); i++) {
                if (board[rowNum - i][colNum + i].getBtnText().equals("-")
                        || board[rowNum - i][colNum + i].getBtnText().equals(word.substring(i, i + 1))) {
                    canNum++;
                }
            }
        }
